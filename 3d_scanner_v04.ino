...
const int ledStartState = HIGH;           // Current state of the output pin
...
void setup() {
  ...
  //stepperMotor.moveTo(4096/2);          // 4096 steps for 1 rotation in 28BYJ-48 – 5V Stepper Motor
...
if (stepperMotor.distanceToGo() == 0) {
  ...
  stepperMotor.run();        // run the stepper to the destined position
  //NOTE:- Whenever the run() is called the stepper is moved only 1 step to the direction assigned....so its important to call it every instance....more checking and looping before the run()is called... slows down the actual speed set initially
}
...
void startRotating(){
  ...
  stepperMotor.moveTo(4096/2);                //Rotate 1 revolution
  stepperMotor.run();                         //Start
}
